...
class Solution3
{
    public:
        int BinarySearch(int *num_list_sorted, int len, int value)
        {
            ...
            if (right + 1 < len && num_list_sorted[right + 1] == value)
                return right + 1;
            else
                return -1;
            // 注意到循环结束之后 left 和 right 的关系一定是 left > right，也就是 left = right + 1。所以也可以用 left 来表示。
        }
};
...
class Solution4
{
    public:
        int BinarySearch(int *num_list_sorted, int len, int value)
        {
            ...
            if (right > -1 && num_list_sorted[right] == value)
                return right;
            else
                return -1;
        }
