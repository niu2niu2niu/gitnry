...
if __name__ == '__main__':
    ...
    (d_machine_list, d_machine_list_unfold) = file_wr.read_machine_list(machine_list_filename)
    print 'd_machine_list:', d_machine_list
    print 'd_machine_list_unfold:', d_machine_list_unfold
    
    # 读当前需要调度的菜名
    dish_name_filename = "dish_name.txt"
    d_dish_name = file_wr.read_dish_name(dish_name_filename)
    print 'd_dish_name:', d_dish_name

    # 读取菜名和任务文件名的对应表
    dish_file_map_filename = "dish_file_map.txt"
    d_dish_file_map = file_wr.read_dish_file_map(dish_file_map_filename)
    ...
    for dish_name in d_dish_name:
        ...
        d_step_info = file_wr.read_dish_file(dish_filename, d_machine_list_unfold)
        print 'd_step_info:', d_step_info
        step_total(d_step_info, d_step_total)
        print 'd_step_total', d_step_total
    file_wr.write_step_total(d_step_total)
